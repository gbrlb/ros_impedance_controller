...
#include <ros_impedance_controller/controller_TaskSpace.h>
#include <string.h>
#include <math.h>
...
namespace ros_impedance_controller
{
    ...
    Controller_TaskSpace::Controller_TaskSpace()
    {
    }
    ...
    bool Controller_TaskSpace::init(hardware_interface::RobotHW *robot_hw,
                          ros::NodeHandle &root_nh,
                          ros::NodeHandle &controller_nh)
    {
        // getting the names of the joints from the ROS parameter server
        std::cout << cyan << "ROS_IMPEDANCE CONTROLLER: Initialize Ros Impedance Controller_TaskSpace" << reset << std::endl;
        root_nh_ = &root_nh;
        assert(robot_hw);
        ...
        command_sub_ = root_nh.subscribe("/command", 1, &Controller_TaskSpace::commandCallback, this, ros::TransportHints().tcpNoDelay());

        std::cout << cyan << "ROS_IMPEDANCE CONTROLLER: ROBOT NAME IS : " << robot_name << reset << std::endl;
        // Create the PID set service
        set_pids_srv_ = param_node.advertiseService("/set_pids", &Controller_TaskSpace::setPidsCallback, this);

        effort_pid_pub = root_nh.advertise<EffortPid>("effort_pid", 1);
        return true;
    }

    void Controller_TaskSpace::starting(const ros::Time &time)
    {
        ROS_DEBUG("Starting Controller_TaskSpace");
    }

    bool Controller_TaskSpace::setPidsCallback(set_pids::Request &req,
    ...
    void Controller_TaskSpace::commandCallback(const sensor_msgs::JointState &msg)
    {
    ...
    void Controller_TaskSpace::baseGroundTruthCB(const nav_msgs::OdometryConstPtr &msg)
    {
    ...
    void Controller_TaskSpace::update(const ros::Time &time, const ros::Duration &period)
    {
        ...
        {
            des_joint_efforts_pids_ = Controller_TaskSpace::control_PD();
    ...
    Eigen::MatrixXd Controller_TaskSpace::fk_leg(Eigen::MatrixXd q)
    {
    ...
    Eigen::MatrixXd Controller_TaskSpace::J_leg_R(Eigen::MatrixXd q)
    {
        Eigen::VectorXd L(3);
        L << 0.08, 0.213, 0.213;
    ...
    Eigen::MatrixXd Controller_TaskSpace::J_leg_L(Eigen::MatrixXd q)
    {
    ...
    Eigen::MatrixXd Controller_TaskSpace::diff_fk_leg(Eigen::MatrixXd q, Eigen::MatrixXd dq)
    {
        ...
        for (unsigned int j = 0; j < 4; ++j)
        {
            if (j == 0 || j == 2)
            {
                ...
                dp_ax = Controller_TaskSpace::J_leg_R(q_ax) * dq_ax;
            ...
            else
            {
                ...
                Eigen::VectorXd dp_ax(3);
                dp_ax = Controller_TaskSpace::J_leg_L(q_ax) * dq_ax;
    ...
    Eigen::VectorXd Controller_TaskSpace::control_PD()
    {
        ...
        Eigen::MatrixXd p = Eigen::MatrixXd::Zero(3, 4);
        p = Controller_TaskSpace::fk_leg(q);
        Eigen::MatrixXd dp = Eigen::MatrixXd::Zero(3, 4);
        dp = Controller_TaskSpace::diff_fk_leg(q, dq);
        ...
        for (unsigned int j = 0; j < 4; ++j)
        {
            if (j == 0 || j == 2)
            {
                ...
                J_leg_inv = Controller_TaskSpace::J_leg_R(q_ax).inverse();
            ...
            else
            {
                ...
                Eigen::MatrixXd J_leg_inv(3, 3);
                J_leg_inv = Controller_TaskSpace::J_leg_L(q_ax).inverse();
    ...
    void Controller_TaskSpace::stopping(const ros::Time &time)
    {
        ROS_DEBUG("Stopping Controller_TaskSpace");
    }
